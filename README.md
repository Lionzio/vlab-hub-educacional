...
## 🧠 Arquitetura e engenharia de software (O "porquê")
...
O servidor foi desenhado em camadas concêntricas (Domain-Driven), garantindo a separação de responsabilidades (SoC):
* **Controllers:** Lidam estritamente com o roteamento HTTP e status codes.
* **Services:** Onde vive a regra de negócio e a integração com a IA externa.
...
* **Schemas:** Contratos rigorosos de validação de dados de entrada e saída (Pydantic V2).
...
* **Hooks customizados:** Toda a lógica de estado, *loading* e chamadas de API do Axios foi isolada dentro do `useMaterials.ts`, mantendo os componentes visuais focados apenas em desenhar a tela.

### ⚙️ DevOps & Qualidade garantida (CI/CD)
* O projeto opera em um Monorepo com ambientes isolados (Pipfile exclusivo no backend, package.json no frontend).
* **Pipeline de CI (GitHub Actions):** Todo código enviado passa pela esteira automatizada que roda o *Black* (Formatador de Estrutura) e o *Flake8* (Linter de Estilo PEP8), garantindo um código imaculado.
* **Testes mockados:** Os testes unitários do Pytest utilizam `unittest.mock` para simular as respostas do Google Gemini. Isso garante a qualidade do código sem gerar custos na cota real da API durante as validações locais ou no pipeline.
...
| **Banco de Dados** | PostgreSQL 15 | Imagem Oficial Docker (Alpine) |
...
│   │   ├── schemas/             # Validação tipada
│   │   └── services/            # Orquestração e conexão com Gemini IA
│   ├── tests/                   # Pytest com mock do SDK
│   ├── Dockerfile               # Imagem otimizada do Python
...
│   │   ├── hooks/               # Regras de negócio do front
│   │   ├── types/               # Tipagens estritas das entidades
│   │   └── api.ts               # Proxy de conexão com o backend
├── docker-compose.yml           # Orquestração do backend e PostgreSQL
└── .env.example                 # Contrato de variáveis de ambiente seguras

🚀 Como executar o projeto localmente
O projeto utiliza contêineres para garantir que rode perfeitamente em qualquer sistema operacional (Windows, Linux ou macOS) sem a temida frase: "Na minha máquina funciona".
Pré-requisitos
Docker e Docker Compose instalados e rodando.
Node.js (v18+) instalado (apenas para rodar o Frontend de desenvolvimento).
...
(O Frontend já está configurado com um proxy interno no vite.config.ts para evitar erros de CORS ao se comunicar com a API do Docker).

🧪 Como rodar os testes unitários
Para garantir a qualidade do sistema sem gastar a cota da API da Inteligência Artificial, os testes foram configurados localmente no backend.
Com o Docker em execução, abra um terminal e execute os testes diretamente dentro do container isolado:
Bash

docker-compose exec backend python -m pytest

Desenvolvido com dedicação, arquitetura escalável e muitas xícaras de café para o processo seletivo do V-Lab. ☕🚀